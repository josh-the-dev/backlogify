...
describe("UserGamesController (e2e)", () => {
	...
	describe("PATCH /user-games/:gameId", () => {
		...
		it("pins a game as Up next", async () => {
			const res = await request(app.getHttpServer())
				.patch(`/user-games/${createdGameId}`)
				.set("x-api-key", TEST_API_KEY)
				.send({ status: "backlog", pinned: true });

			expect(res.status).toBe(200);
			expect(res.body.pinnedAt).toEqual(expect.any(String));
		});

		it("pinning another game steals the Up next slot", async () => {
			const createRes = await request(app.getHttpServer())
				.post("/user-games")
				.set("x-api-key", TEST_API_KEY)
				.send({
					externalServiceId: "rawg-67890",
					name: "E2E Second Game",
					status: "backlog",
				});
			const secondGameId = createRes.body.id;

			const pinRes = await request(app.getHttpServer())
				.patch(`/user-games/${secondGameId}`)
				.set("x-api-key", TEST_API_KEY)
				.send({ pinned: true });

			expect(pinRes.status).toBe(200);
			expect(pinRes.body.pinnedAt).toEqual(expect.any(String));

			const listRes = await request(app.getHttpServer())
				.get("/user-games")
				.set("x-api-key", TEST_API_KEY);
			const firstGame = listRes.body.find(
				(g: { id: string }) => g.id === createdGameId,
			);
			expect(firstGame.pinnedAt).toBeNull();
		});

		it("clears the pin when status moves to played", async () => {
			await request(app.getHttpServer())
				.patch(`/user-games/${createdGameId}`)
				.set("x-api-key", TEST_API_KEY)
				.send({ pinned: true });
		...
		it("unpins with pinned false", async () => {
			await request(app.getHttpServer())
				.patch(`/user-games/${createdGameId}`)
				.set("x-api-key", TEST_API_KEY)
				.send({ pinned: true });
